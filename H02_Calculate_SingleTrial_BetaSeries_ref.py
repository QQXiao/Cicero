...
class pybetaseries():
    ...
    def __init__(self,fsfdir,tempderiv,motpars,time_res,bold_dir,bold_file):
        ...
        if not bold_dir.endswith('/'):
            bold_dir=''.join([bold_dir,'/'])
        ...
        self.max_evtime=self.TR*self.ntp - self.TR
        ...
        self.raw_data=fmri_dataset(bold_dir + bold_file,mask=maskimg)
...
def otherevs(whichevs,numorigev,tempderiv,motpars):
        ...
        if tempderiv:
            ...
            if motpars == 1:
                ...
                # in Feat 6.00, if we used Standard + Extend Motion Parameters,
                # there's 24 motion parameters added to the design matrix
                nuisance.extend(range(2*numorigev,(24+2*numorigev)))
            ...
            if motpars == 1:
                ...
                nuisance.extend(range(numorigev,24+numorigev))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--motpars',dest='motpars',type=int,default=0,
                        help='Motion parameters indicator. \
                        If 0, there is no motion parameters include. \
                        If 1, The code assumes that motion parameters are the first 6 EVs after the real EVs in the Feat design matrix. \
                        If 2, The code assumes that motion parameters are the first 24 EVs after the real EVs in the Feat design matrix.\
                        If you have other motion parameter which does not need to convolve, you need to hijack the otherevs parts and workaround.')
    ...
    parser.add_argument('--bold_dir',dest='bold_dir',default='',help='Path to Target bold Directory')
    parser.add_argument('--bold_file',dest='bold_file',default='',help='Path to Target bold file')

    args = parser.parse_args()

    # create instance
    pybeta = pybetaseries(args.fsldir,args.tempderiv,args.motpars,args.timeres,args.bold_dir,args.bold_file)
    if args.lss:
