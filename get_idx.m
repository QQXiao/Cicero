...
% load testing data
    ...
    for k = 2:TTN
        ...
        check_item = [check_item, ismember(behav.WordID(k:TTN),behav.WordID(k-1))'];
    ...
    %%% get indexes
    %%1. %item-specific ERS
    idx_item_ERS{1}=find(idx_phase1==1 & check_phase==0 & check_run==1 & ...
    check_item==1 & check_mem2<=xm);
    idx_item_ERS{2}=find(idx_phase1==1 & check_phase==0 & check_run==1 & ...
    check_item==0 & check_mem2<=xm & dis_loci==0);
    ...
    for c=1:2 %1=within run;2=cross runs
        %encoding
        idx_rs{1,c,1}=find(idx_phase1==1 & check_phase==1 & ...
        check_run==2-c & dis_loci==0);
        idx_rs{1,c,2}=find(idx_phase1==1 & check_phase==1 & ...
        check_run==2-c & check_lociCate==1 & ...
        dis_loci~=0 & ...
        ((dis_ESeq >= 8 & dis_ESeq <= 12) | (dis_ESeq >= 18 & dis_ESeq <= 22)));
        %retrieval
        idx_rs{2,c,1}=find(idx_phase1==2 & check_phase==1 & ...
        check_run==2-c & dis_loci==0 & ...
        check_mem1<=xm & check_mem2<=xm);
        idx_rs{2,c,2}=find(idx_phase1==2 & check_phase==1 & ...
        check_run==2-c & check_lociCate==1 & ...
        dis_loci~=0 & ...
        check_mem1<=xm & check_mem2<=xm & ...
        ((dis_ESeq >= 8 & dis_ESeq <= 12) | (dis_ESeq >= 18 & dis_ESeq <= 22)));
        idx_rs{2,c,3}=find(idx_phase1==2 & check_phase==1 & ...
        check_run==2-c & dis_loci==0 & ...
        check_mem1==10 & check_mem2<=xm);
        %ERS
        idx_rs{3,c,1}=find(idx_phase1==1 & check_phase==0 & ...
        check_run==2-c & dis_loci==0 & ...
        check_mem2<=xm);
        idx_rs{3,c,2}=find(idx_phase1==1 & check_phase==0 & ...
        check_run==2-c & check_lociCate==1 & ...
        dis_loci~=0 & ...
        check_mem2<=xm & ...
        ((dis_ESeq >= 8 & dis_ESeq <= 12) | (dis_ESeq >= 18 & dis_ESeq <= 22)));
        idx_rs{3,c,3}=find(idx_phase1==1 & check_phase==0 & ...
        check_run==2-c & dis_loci==0 & ...
        check_mem2==10);
    end
    ...
    for d=1:2 %1:distance = 10;2:distance=20
        idx_rt_ln{d}=find(idx_phase1==1 & check_phase==1 & check_run==1 & dis_loci==0 & dis_ESeq==10*d);
        idx_rt_mem{d}=find(idx_phase1==2 & check_phase==1 & check_run==1 & dis_loci==0 & dis_ESeq==10*d & check_mem1<=xm & check_mem2<=xm);
        idx_rt_ERS{d}=find(idx_phase1==1 & check_phase==0 & check_run==1 & dis_loci==0 & dis_ESeq==10*d & check_mem2<=xm);
    end
      %ed_st=[1,4,8];ed_end=[3,7,29];
      %for ed=1:length(ed_st);
      %    idx_rt{ed}=find(idx_phase1==2 & check_phase==1 & check_run==1 & dis_ESeq>=ed_st(ed) & dis_ESeq<=ed_end(ed) & dis_RSeq<=3 & check_mem1<=xm & check_mem2<=xm);
      %end
      llist=[];slist=[];
      xm=1;
      for iid=2:30;
          for d=1:15;
                p1=find(idx_phase1==2 & check_phase==1 & check_run==1 & idx_item1==iid-d & idx_item2==iid & check_mem1<=xm & check_mem2<=xm);
                p2=find(idx_phase1==2 & check_phase==1 & check_run==1 & idx_item1==iid & idx_item2==iid+d & check_mem1<=xm & check_mem2<=xm);
                p_set=[p1,p2];
                if dis_ESeq(p1)~=dis_ESeq(p2);
                    [tmp,l]=max(dis_ESeq(p_set));
                    [tmp,s]=min(dis_ESeq(p_set));
                    llist=[llist p_set(l)];slist=[slist p_set(s)];
                end
          end
      end
      idx_rt{1}=slist;
