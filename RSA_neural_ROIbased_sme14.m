...
%get fMRI data
    ...
    %%analysis
        ...
        rs_SME_ERS(roi,3)=mean(cc(idx_sme{3,1,4,1}));%wrong wl
        rs_SME_ERS(roi,4)=mean(cc(idx_sme{3,1,4,2}));%wrong bl
   end %end roi
    rs_SME_ERS_z=0.5*(log(1+rs_SME_ERS)-log(1-rs_SME_ERS));

eval(sprintf('save %s/rs_SME_ERS14_sub%02d rs_SME_ERS_z', zdir, iSub));
end %end func
