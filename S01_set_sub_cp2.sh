...
cd $datadir
#for i in MOL13* MOL14* MOL16* MOL19* MOL22*
for i in MOL30*
#for i in *.tar.gz
do
        dirname=`echo $i`
		...
		#coronal=`ls *T2coronals030*`
		coronal=`ls *T2coronals032*`
		fslreorient2std $coronal ${anatomydir}/coronal_ND.nii.gz
		#coronal=`ls *T2coronals031*`
		coronal=`ls *T2coronals033*`
	...
	#	fsl_prepare_fieldmap SIEMENS ${filedmapdir}/fieldmap_phase ${filedmapdir}/fieldmap_mag_brain ${filedmapdir}/fieldmap_rads 2.46
		...
		#fsl_sub bet 3d.nii.gz 3d_brain_tight.nii.gz -m -f 0.3 -R -B
       	fi
done
