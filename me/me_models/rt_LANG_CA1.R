...
item_file <- paste(basedir,"pattern/ROI_based/ref_space/me/rt_mem_LANG.txt",sep="/")
...
rt_Eact <- lmer(rt~HEact+(1+HEact|subid),REML=FALSE,data=subdata)
rt_Eact.null <- lmer(rt~1+(1+HEact|subid),REML=FALSE,data=subdata)
rt_Ract <- lmer(rt~HRact+(1+HRact|subid),REML=FALSE,data=subdata)
rt_Ract.null <- lmer(rt~1+(1+HRact|subid),REML=FALSE,data=subdata)
