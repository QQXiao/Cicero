...
item_file <- paste(basedir,"pattern/ROI_based/ref_space/me/rs_mem_LANG.txt",sep="/")
item_data <- read.table(item_file,header=FALSE)
act_file <- paste(basedir,"pattern/ROI_based/ref_space/me/act_LANG.txt",sep="/")
...
subhip_file <- paste(basedir,"pattern/ROI_based/ref_space/me/act_CA1.txt",sep="/")
...
rt_Eact <- lmer(rs~HEact+(1+HEact|subid),REML=FALSE,data=subdata)
rt_Eact.null <- lmer(rs~1+(1+HEact|subid),REML=FALSE,data=subdata)
rt_Ract <- lmer(rs~HRact+(1+HRact|subid),REML=FALSE,data=subdata)
rt_Ract.null <- lmer(rs~1+(1+HRact|subid),REML=FALSE,data=subdata)
...
mainEffect.rs_Ract <- anova(rt_Ract,rs_Ract.null)
