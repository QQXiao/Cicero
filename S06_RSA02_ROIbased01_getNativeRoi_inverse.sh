...
#WarpImageMultiTransform 3 ${roi} $resultdir/${roi_prefix}.nii.gz -R $reffile -i $affinedir/sub${SUB}_Affine.txt
fsl_sub -q verylong.q WarpImageMultiTransform 3 ${roi} $resultdir/${roi_prefix}.nii.gz -R $reffile -i $affinedir/sub${SUB}_Affine.txt
done
done
